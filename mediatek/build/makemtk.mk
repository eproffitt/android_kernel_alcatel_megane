...
include device/jrdcom/diablo/ics_diablo.mk
...
.PHONY : new newall remake remakeall clean cleanall \
         ...
         check-modem update-modem sign-image sign-perso encrypt-image sign-modem check-dep \
...
ifeq ($(ENABLE_TEE), TRUE)
  ...
  DEAL_STDOUT_SIGN_PERSO := 2>&1 | tee -a $(LOG)sign-perso.log
  ...
  DEAL_STDOUT_SIGN_PERSO := > $(LOG)sign-perso.log 2>&1
...
copy-modem:
	@rm -R $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM)) 
	@cp -r $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG) $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM))
...
perso:  clean-modem custgen javaoptgen ptgen check-modem sign-modem
	$(hide) echo $(SHOWTIME) $@ing...
	$(hide) /usr/bin/perl mediatek/build/tools/mtkBegin.pl $(PROJECT)
	$(MAKEPERSO) $(DEAL_STDOUT_PERSO) && \
	$(SHOWRSLT) $$? $(LOG)$@.log || \
	$(SHOWRSLT) $$? $(LOG)$@.log
...
sign-perso:
	$(hide) echo $(SHOWTIME) $@ing ...
	$(hide) echo -e \\t\\t\\t\\b\\b\\b\\bLOG: $(S_LOG)$@.log
	$(hide) perl mediatek/build/tools/SignTool/SignTool_perso.pl $(PROJECT) $(FULL_PROJECT) $(MTK_SEC_SECRO_AC_SUPPORT) $(MTK_NAND_PAGE_SIZE) $(DEAL_STDOUT_SIGN_PERSO) && \
	  $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log || \
          $(SHOWRSLT) $${PIPESTATUS[0]} $(LOG)$@.log
...
check-modem: PRIVATE_DSP_BL := \
             $(if $(wildcard $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG)/DSP_BL), \
               $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG)/DSP_BL, \
               $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM))/DSP_BL \
              )
check-modem: PRIVATE_DSP_ROM := \
             $(if $(wildcard $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG)/DSP_ROM), \
               $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG)/DSP_ROM, \
               $(MTK_ROOT_CUSTOM)/common/modem/$(strip $(CUSTOM_MODEM))/DSP_ROM \
              )
check-modem: PRIVATE_MODEM := \
             $(if $(wildcard $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG)/modem.img), \
               $(JRD_WIMDATA)/wcustores/Modem/$(MODEM_IMG)/modem.img, \
