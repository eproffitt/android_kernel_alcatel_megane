...
#ifdef T4K04_DEBUG_SOFIA
	#define T4K04DBSOFIA(fmt, arg...) printk( "[T4K04Raw] "  fmt, ##arg)
#else
	#define T4K04DBSOFIA(x,...)
#endif
...
MSDK_SCENARIO_ID_ENUM T4K04CurrentScenarioId = ACDK_SCENARIO_ID_CAMERA_PREVIEW;
...
void T4K04_write_shutter(kal_uint32 shutter)
{
	...
	T4K04DBSOFIA("!!shutter=%d!!!!!\n", shutter);
	
	if(t4k04.T4K04AutoFlickerMode == KAL_TRUE)
	{
		if ( SENSOR_MODE_PREVIEW == t4k04.sensorMode )  //(g_iT4K04_Mode == T4K04_MODE_PREVIEW)	
		{
		...
		switch(T4K04CurrentScenarioId)
		{
        	case MSDK_SCENARIO_ID_CAMERA_ZSD:
				T4K04DBSOFIA("AutoFlickerMode!!! MSDK_SCENARIO_ID_CAMERA_ZSD  0!!\n");
		...
		T4K04DBSOFIA("AutoFlickerMode!!! min_framelength for AutoFlickerMode = %d (0x%x)\n",min_framelength,min_framelength);
		T4K04DBSOFIA("max framerate(10 base) autofilker = %d\n",(t4k04.pvPclk*10000)*10 /line_length/min_framelength);
		...
		if (frame_length < min_framelength)
		{
        	...
        	case MSDK_SCENARIO_ID_CAMERA_ZSD:
				...
				T4K04DB("AutoFlickerMode!!! MSDK_SCENARIO_ID_CAMERA_ZSD  1  extra_lines!!\n",extra_lines);
				break;
			default:
		...
		T4K04DBSOFIA("frame_length 2= %d\n",frame_length);
		T4K04DB("framerate(10 base) = %d\n",(t4k04.pvPclk*10000)*10 /line_length/frame_length);
		
		T4K04DB("shutter=%d, extra_lines=%d, line_length=%d, frame_length=%d\n", shutter, extra_lines, line_length, frame_length);
	
	}
	else
	{
		if ( SENSOR_MODE_PREVIEW == t4k04.sensorMode )  //(g_iT4K04_Mode == T4K04_MODE_PREVIEW)	//SXGA size output
		{
			max_shutter = T4K04_PV_PERIOD_LINE_NUMS + t4k04.DummyLines ; 
		}
		else	
...
static kal_uint16 T4K04Reg2Gain(const kal_uint16 iReg)
{
    ...
    sensorGain = iReg/0x41;  //get sensor gain multiple
...
{
	T4K04DB("T4K04_Sensor_Init enter :\n ");	
	...
	T4K04DB("T4K04_Sensor_Init exit :\n ");	
...
{	
	...
	// Extclk= 24Mhz  PCK = 130.2Mhz   DCLK = 81.375MHz  MIPI Lane CLK = 325.5MHz  ES 1 line = 26.2058us
	// FPS = 30.19fps   Image output size = 1640x1232
	T4K04DB("T4K04PreviewSetting enter :\n ");	
	...
	T4K04DB("T4K04PreviewSetting exit :\n ");
}

void T4K04CaptureSetting(void)
{	
    T4K04DB("T4K04CaptureSetting enter :\n ");
	// ==========Summary========
	...
	T4K04DB("T4K04CaptureSetting exit :\n ");
}
...
UINT32 T4K04GetSensorID(UINT32 *sensorID)
{
    ...
    do {
        //*sensorID = (T4K04_read_cmos_sensor(0x300A)<<8)|T4K04_read_cmos_sensor(0x300B);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	// preview size 
	if(t4k04.sensorMode == SENSOR_MODE_PREVIEW) 
	{
		// do nothing	
		// FOR CCT PREVIEW
	}
	...
	T4K04_write_shutter(t4k04.shutter);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	{
	//shutter=T4K04_read_shutter();
	//temp_data =  read_T4K04_gain();
	spin_lock(&t4k04mipiraw_drv_lock);
	t4k04.pvShutter =shutter;
	//t4k04.sensorGlobalGain = temp_data;
	//t4k04.pvGain =t4k04.sensorGlobalGain;
	spin_unlock(&t4k04mipiraw_drv_lock);	
	// Full size setting
	T4K04CaptureSetting();
	}
	
	//T4K04DB("[T4K04Capture]t4k04.shutter=%d, read_pv_shutter=%d, read_pv_gain = 0x%x\n",t4k04.shutter, shutter,t4k04.sensorGlobalGain);
	...
	t4k04.DummyLines  =0;
	spin_unlock(&t4k04mipiraw_drv_lock);
	//T4K04DB("[T4K04Capture] mirror&flip: %d\n",sensor_config_data->SensorImageMirror);
	T4K04SetFlipMirror(sensor_config_data->SensorImageMirror);

	//T4K04_SetDummy( t4k04.DummyPixels, t4k04.DummyLines);
	
    if(T4K04CurrentScenarioId==MSDK_SCENARIO_ID_CAMERA_ZSD)
    {
		T4K04DB("T4K04Capture exit ZSD!!\n");
		return;
    }
	
	//calculate shutter
	pv_line_length = T4K04_PV_PERIOD_PIXEL_NUMS + t4k04.DummyPixels;
	cap_line_length = T4K04_FULL_PERIOD_PIXEL_NUMS + t4k04.DummyPixels;	
	
	T4K04DB("[T4K04Capture]pv_line_length =%d,cap_line_length =%d\n",pv_line_length,cap_line_length);
	T4K04DB("[T4K04Capture]pv_shutter =%d\n",shutter );
	
	shutter =  shutter * pv_line_length / cap_line_length;
	shutter = shutter *t4k04.capPclk / t4k04.pvPclk;
	
	if(shutter < 1)
	    shutter = 1;	

	T4K04_write_shutter(shutter);	
	
	T4K04DB("[T4K04Capture]cap_shutter =%d , cap_read gain = 0x%x\n",shutter,read_T4K04_gain());
...
{	
	...
	pSensorInfo->SensorPreviewResolutionX= T4K04_IMAGE_SENSOR_PV_WIDTH; 
    pSensorInfo->SensorPreviewResolutionY= T4K04_IMAGE_SENSOR_PV_HEIGHT; 
	pSensorInfo->SensorFullResolutionX= T4K04_IMAGE_SENSOR_FULL_WIDTH;
    pSensorInfo->SensorFullResolutionY= T4K04_IMAGE_SENSOR_FULL_HEIGHT;
    ...
    switch (ScenarioId)
    {
		...
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
            ...
            pSensorInfo->SensorGrabStartX = T4K04_FULL_X_START;	//2*T4K04_IMAGE_SENSOR_PV_STARTX; 
            pSensorInfo->SensorGrabStartY = T4K04_FULL_Y_START;	//2*T4K04_IMAGE_SENSOR_PV_STARTY;    
...
UINT32 T4K04SetVideoMode(UINT16 u2FrameRate)//checkd
{
    ...
    if(t4k04.sensorMode == SENSOR_MODE_PREVIEW)
    {
    	if(t4k04.T4K04AutoFlickerMode == KAL_TRUE)
    	{
			...
			MAX_Frame_length = (t4k04.pvPclk*10000)/(T4K04_PV_PERIOD_PIXEL_NUMS + t4k04.DummyPixels)/frameRate*10;
    	}
		else
			MAX_Frame_length = (t4k04.pvPclk*10000) /(T4K04_PV_PERIOD_PIXEL_NUMS + t4k04.DummyPixels)/u2FrameRate;
				
		if((MAX_Frame_length <=T4K04_PV_PERIOD_LINE_NUMS))
		{
			MAX_Frame_length = T4K04_PV_PERIOD_LINE_NUMS;
			T4K04DB("[T4K04SetVideoMode]current fps = %d\n", (t4k04.pvPclk*10000)  /(T4K04_PV_PERIOD_PIXEL_NUMS)/T4K04_PV_PERIOD_LINE_NUMS);	
		}
		T4K04DB("[T4K04SetVideoMode]current fps (10 base)= %d\n", (t4k04.pvPclk*10000)*10/(T4K04_PV_PERIOD_PIXEL_NUMS + t4k04.DummyPixels)/MAX_Frame_length);
		extralines = MAX_Frame_length - T4K04_PV_PERIOD_LINE_NUMS;
	
		T4K04_SetDummy(t4k04.DummyPixels,extralines);
    }
	else if(t4k04.sensorMode == SENSOR_MODE_CAPTURE)//also could be used in 1080p video-record
	{
		T4K04DB("-------[T4K04SetVideoMode]ZSD or 1080p Record---------\n");
		if(t4k04.T4K04AutoFlickerMode == KAL_TRUE)
    	{
			if (u2FrameRate==15)
				frameRate= 148;
				...
				frameRate=u2FrameRate*10;
			MAX_Frame_length = (t4k04.capPclk*10000) /(T4K04_FULL_PERIOD_PIXEL_NUMS + t4k04.DummyPixels)/frameRate*10;
    	}
		else
			MAX_Frame_length = (t4k04.capPclk*10000) /(T4K04_FULL_PERIOD_PIXEL_NUMS + t4k04.DummyPixels)/u2FrameRate;
		
		if((MAX_Frame_length <=T4K04_FULL_PERIOD_LINE_NUMS))
		{
			MAX_Frame_length = T4K04_FULL_PERIOD_LINE_NUMS;
			T4K04DB("[T4K04SetVideoMode]current fps = %d\n", (t4k04.capPclk*10000) /(T4K04_FULL_PERIOD_PIXEL_NUMS)/T4K04_FULL_PERIOD_LINE_NUMS);
		
		}
		T4K04DB("[T4K04SetVideoMode]current fps (10 base)= %d\n", (t4k04.capPclk*10000)*10/(T4K04_FULL_PERIOD_PIXEL_NUMS + t4k04.DummyPixels)/MAX_Frame_length);
		
		extralines = MAX_Frame_length - T4K04_FULL_PERIOD_LINE_NUMS;
...
UINT32 T4K04SetAutoFlickerMode(kal_bool bEnable, UINT16 u2FrameRate)
{
    ...
    T4K04DB("[T4K04SetAutoFlickerMode] frame rate(10base) = %d %d\n", bEnable, u2FrameRate);
	if(bEnable) {
		spin_lock(&t4k04mipiraw_drv_lock);
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_PERIOD:
			switch(T4K04CurrentScenarioId)
			{
				...
				case MSDK_SCENARIO_ID_CAMERA_ZSD:
					T4K04DB("Sensor period:MSDK_SCENARIO_ID_CAMERA_ZSD\n");
        ...
        case SENSOR_FEATURE_GET_PIXEL_CLOCK_FREQ:
			...
			//T4K04DB("T4K04FeatureControl:T4K04CurrentScenarioId:%d\n",T4K04CurrentScenarioId);
				case MSDK_SCENARIO_ID_CAMERA_ZSD:
					T4K04DB("PIXEL_CLOCK_FREQ:MSDK_SCENARIO_ID_CAMERA_ZSD\n");
					*pFeatureReturnPara32 = 130200000; 		            	 		  
            	 	*pFeatureParaLen=4;		         	
         			 break;
         		default:
            		*pFeatureReturnPara32 = 130200000; 
