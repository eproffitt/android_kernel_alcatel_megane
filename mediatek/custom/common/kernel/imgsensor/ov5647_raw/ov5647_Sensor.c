...
static void OV5647_Write_Shutter(kal_uint16 iShutter)
{
	...
	OV5647_TRACE("[OV5647_Write_Shutter]iShutter=%d",iShutter);
...
UINT32 OV5647SetVideoMode(UINT16 u2FrameRate)
{
	...
	if(u2FrameRate == 30){
		...
		spin_unlock(&ov5647_drv_lock);
		OV5647_TRACE("[OV5647SetVideoMode] error framerate!!! : u2FrameRate = %d", u2FrameRate);
		return ERROR_NONE;
	}
	...
	if((u2FrameRate == 30)&&(OV5647AutoFlicKerMode==KAL_TRUE))
		u2FrameRate = 296;
	else if((u2FrameRate == 15)&&(OV5647AutoFlicKerMode==KAL_TRUE))
		u2FrameRate = 148;
	else
...
{
	OV5647_TRACE("[OV5647SetAutoFlickerMode]bEnable=%d, u2FrameRate=%d",bEnable,u2FrameRate);
	if(bEnable){
		spin_lock(&ov5647_drv_lock);
		OV5647AutoFlicKerMode = KAL_TRUE;
		spin_unlock(&ov5647_drv_lock);
		/*Change frame rate 29.5fps to 29.8fps to do Auto flick*/
		if(OV5647_sensor.video_mode==KAL_TRUE){
			if(OV5647_sensor.FixedFps == 30)
				OV5647SetMaxFrameRate(296);
			else if(OV5647_sensor.FixedFps == 15)
				OV5647SetMaxFrameRate(148);
		}
