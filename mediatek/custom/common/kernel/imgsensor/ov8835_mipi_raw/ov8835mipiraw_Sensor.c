...
#define SENSORDB(x,...)
#endif
#define AWB_GOLDEN_SAMPLE_FIX 1
//#define ACDK
...
unsigned long OTP_DelayTime=10;  // spec. is 10sec,3sec is work,test by norman
kal_uint16 OTP_index=6;//used=0~3,no_calibrate=5,init=6
...
static OV8835MIPI_sensor_struct OV8835MIPI_sensor =
{
  ...
  .frame_height = OV8835MIPI_PV_PERIOD_LINE_NUMS,
...
kal_uint16 ov8835mipi_check_otp_flag(kal_uint16 index)
{
    kal_uint16 temp,flag;
    kal_uint32 address;

    //select bank 
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+1+index*7);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
    ...
    printk("OV8835MIPI_check_otp_flag=%d\r\n",flag);

    if(NULL == flag)
    {
        SENSORDB("[OV8835MIPI_check_otp]index[%x]read flag[%x][0]\n",index,flag);
        return 0;
    }
    else if((!(flag&0x80)) && (flag&0x7f))
    {
        SENSORDB("[OV8835MIPI_check_otp]index[%x]read flag[%x][1]\n",index,flag);
        return 1;
    }
...
static void  ov8835mipi_check_otp_CRC(kal_uint16 index)
{
    kal_uint16 CRC0,CRC1;
    kal_uint32 CRC,address;

    //select bank 
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+7+index*7);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
    //disable otp read
    ...
    address = 0x3d0e;
    CRC0 = OV8835MIPI_read_cmos_sensor(address);
    address++;
    CRC1 = OV8835MIPI_read_cmos_sensor(address);

    CRC = CRC0 | (CRC1<<8);

    SENSORDB("[OV8835MIPI_read_otp_CRC]index[%x]CRC[%x]\n",index,CRC);
}

//index:index of otp group.(0,1,2,3)
//return value:	0 check success
//				1 no 	OTP
kal_uint16 ov8835mipi_check_otp_Index(void)
{
    kal_uint16 temp,i;
    ...
    if(OTP_index==6)
    {
        for(i = 3; i >= 0; i--)
        {
            temp = ov8835mipi_check_otp_flag(i);
            if(1 == temp)
            {
                OTP_index = i;
                return 0;
        ...
        if( temp == 0)
        {
            SENSORDB("[ov8835mipi_check_otp_Index]no valid wb OTP data!\r\n");
            OTP_index=5;
        }
        return 1;
    }
...
kal_uint16 ov8835mipi_read_otp_wb(kal_uint16 index, struct OV8835MIPI_otp_struct *otp)
{
    ...
    address = 0x3d09;
    otp->lens_id = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]lens_id[%x]\n",address,otp->lens_id);
    
    address++;
    otp->module_integrator_id = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]module_integrator_id[%x]\n",address,otp->module_integrator_id);

    //select bank 
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+3+index*7);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
    //disable otp read
    ...
    address = 0x3d00;
    otp->Unit_AWB_R  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]Unit_AWB_R[%x]\n",address,otp->Unit_AWB_R);

    address++;
    otp->Unit_AWB_Gr  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]Unit_AWB_Gr[%x]\n",address,otp->Unit_AWB_Gr);

    address++;
    otp->Unit_AWB_Gb  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]Unit_AWB_Gb[%x]\n",address,otp->Unit_AWB_Gb);

    address++;
   otp->Unit_AWB_B  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]>Unit_AWB_[%x]\n",address,otp->Unit_AWB_B);
  
    #if AWB_GOLDEN_SAMPLE_FIX
    otp->Gloden_AWB_R=110;
    otp->Gloden_AWB_Gr=194;
    otp->Gloden_AWB_Gb=194;
    otp->Gloden_AWB_B=100;
    SENSORDB("[OV8835MIPI_read_otp_wb]Gloden_AWB_R][%x]\n",otp->Gloden_AWB_R);
    SENSORDB("[OV8835MIPI_read_otp_wb]Gloden_AWB_Gr][%x]\n",otp->Gloden_AWB_Gr);
    SENSORDB("[OV8835MIPI_read_otp_wb]Gloden_AWB_Gb[%x]\n",otp->Gloden_AWB_Gb);
    SENSORDB("[OV8835MIPI_read_otp_wb]Gloden_AWB_B[%x]\n",otp->Gloden_AWB_B);
    #else
    address++;
    otp->Gloden_AWB_R  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]Gloden_AWB_R][%x]\n",address,otp->Gloden_AWB_R);
    address++;
    otp->Gloden_AWB_Gr  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]Gloden_AWB_Gr][%x]\n",address,otp->Gloden_AWB_Gr);
    address++;
    otp->Gloden_AWB_Gb  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]Gloden_AWB_Gb[%x]\n",address,otp->Gloden_AWB_Gb);
    address++;
    otp->Gloden_AWB_B  = OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[OV8835MIPI_read_otp_wb]address[%x]Gloden_AWB_B[%x]\n",address,otp->Gloden_AWB_B);
    #endif
...
kal_uint16 ov8835mipi_read_otp_lenc(kal_uint16 index,struct OV8835MIPI_otp_struct *otp)
{
    ...
    //select bank 
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+4+index*7);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
    //disable otp read
    ...
    for(i = 0; i < 16; i++)
    {	
        otp->lenc[i] = OV8835MIPI_read_cmos_sensor(address+i);
        SENSORDB("[OV8835MIPI_read_otp_lenc]address[%x]otp->lenc[%d][%x]\n",address+i,i,otp->lenc[i]);
    }

    //select bank 
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+5+index*7);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
    //disable otp read
    ...
    for(i = 0; i < 16; i++)
    {	
        otp->lenc[16+i] = OV8835MIPI_read_cmos_sensor(address+i);
        SENSORDB("[OV8835MIPI_read_otp_lenc]address[%x]otp->lenc[%d][%x]\n",address+i,16+i,otp->lenc[16+i]);
    }

    //select bank 
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+6+index*7);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
    //disable otp read
    ...
    for(i = 0; i < 16; i++)
    {	
        otp->lenc[32+i] = OV8835MIPI_read_cmos_sensor(address+i);
    ...
    for(i = 0; i < 14; i++)
    {	
        otp->lenc[48+i] = OV8835MIPI_read_cmos_sensor(address+i);
        SENSORDB("[OV8835MIPI_read_otp_lenc]address[%x]otp->lenc[%d][%x]\n",address+i,48+i,otp->lenc[48+i]);
    }
    
    return 0;
}
kal_uint16 ov8835mipi_read_otp_blc(struct OV8835MIPI_otp_struct *otp)
{
    kal_uint16 temp,i;
    kal_uint32 address;

    //select bank 0
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+31);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
...
kal_uint16 ov8835mipi_read_otp_af(kal_uint16 index,struct OV8835MIPI_otp_struct *otp)
{
    kal_uint16 bank,temp1,temp2,i;
    kal_uint32 address;

    //select bank 
    OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+2+index*7);
    //load otp to buffer
    OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
    msleep(OTP_DelayTime);
    //disable otp read
    ...
    address = 0x3d0c;
    otp->user_data[1] = OV8835MIPI_read_cmos_sensor(address);
    address++;
    otp->user_data[1] = (otp->user_data[1]<<8) | OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[ov8835mipi_read_otp_af]otp->user_data[1][%x]\n",otp->user_data[1]);

    address++;
    otp->user_data[2] = OV8835MIPI_read_cmos_sensor(address);
    address++;
    otp->user_data[2] = (otp->user_data[2]<<8) | OV8835MIPI_read_cmos_sensor(address);
    SENSORDB("[ov8835mipi_read_otp_af]otp->user_data[2][%x]\n",otp->user_data[2]);
    return 0;
}
...
kal_uint16 ov8835mipi_update_wb_gain(kal_uint32 R_gain, kal_uint32 G_gain, kal_uint32 B_gain)
{
    ...
    if(R_gain > 0x400)
    {
        OV8835MIPI_write_cmos_sensor(0x3400,R_gain >> 8);
        OV8835MIPI_write_cmos_sensor(0x3401,(R_gain&0x00ff));
    }
    if(G_gain > 0x400)
    {
        OV8835MIPI_write_cmos_sensor(0x3402,G_gain >> 8);
        OV8835MIPI_write_cmos_sensor(0x3403,(G_gain&0x00ff));
    }
    if(B_gain >0x400)
    {
        OV8835MIPI_write_cmos_sensor(0x3404,B_gain >> 8);
        OV8835MIPI_write_cmos_sensor(0x3405,(B_gain&0x00ff));
    }
    return 0;
}
...
kal_uint16 ov8835mipi_update_lenc(struct OV8835MIPI_otp_struct *otp)
{
    ...
    for(i = 0; i < 62; i++)
    {
        ...
        SENSORDB("[OV8835MIPI_update_lenc]otp->lenc[%x][%x]\n",0x5800+i,OV8835MIPI_read_cmos_sensor(0x5800+i));
    }
    return 0;
}
...
kal_uint32 RG_Ratio_typical;
kal_uint32 BG_Ratio_typical ;
...
kal_uint16 ov8835mipi_update_wb_register_from_otp(void)
{
    ...
    printk("OV8835MIPI_update_wb_register_from_otp");

    //update white balance setting from OTP
    //check first wb OTP with valid OTP

    if(ov8835mipi_check_otp_Index())    return 1;

    if(ov8835mipi_read_otp_wb(OTP_index,&current_otp))    return 1;
    	
    RG_Ratio_typical = (1000*current_otp.Gloden_AWB_R)/current_otp.Gloden_AWB_Gr;
    BG_Ratio_typical = (1000*current_otp.Gloden_AWB_B)/current_otp.Gloden_AWB_Gr;
    ...
    SENSORDB("[OTP_AWB]RG_Ratio_typical=%d\n",RG_Ratio_typical);

    SENSORDB("[OTP_AWB]BG_Ratio_typical=%d\n",BG_Ratio_typical);
...
kal_uint16 ov8835mipi_update_lenc_register_from_otp(void)
{
    ...
    temp = OV8835MIPI_read_cmos_sensor(0x5000);
    temp |= 0x80;
    OV8835MIPI_write_cmos_sensor(0x5000,temp);
...
kal_uint16 ov8835mipi_update_blc_register_from_otp(void)
{
    ...
    ov8835mipi_read_otp_blc(&current_otp);
...
kal_uint16 ov8835mipi_update_af_from_otp(void)
{
    kal_uint16 temp,i;
    struct OV8835MIPI_otp_struct current_otp;
    ...
    ov8835mipi_read_otp_af(OTP_index,&current_otp);

    //success
    return 0;
}
void ov8835mipi_read_all_register_from_otp(void)
{
    ...
    {
        //select bank 
        OV8835MIPI_write_cmos_sensor(0x3d84,0xc0+i);
        //load otp to buffer
        OV8835MIPI_write_cmos_sensor(0x3d81,0x01);
        msleep(OTP_DelayTime);
        //disable otp read
        ...
        address = 0x3d00;
        for(j=0;j<16;j++)
            SENSORDB("[ov8835mipi_read_all_register_from_otp][%02d][%02d]-%02x\n",i,j,OV8835MIPI_read_cmos_sensor(0x3d00+j));
    }
}
#endif
...
unsigned long flags;
   ...
   if (OV8835MIPI_sensor.pv_mode)
	{
	  
	  frame_length = OV8835MIPI_PV_PERIOD_LINE_NUMS + OV8835MIPI_sensor.dummy_lines;
	}
	else
	{
	  
	  frame_length = OV8835MIPI_FULL_PERIOD_LINE_NUMS + OV8835MIPI_sensor.dummy_lines;
	}
	...
	if (OV8835MIPI_sensor.pv_mode)
	{
	  ...
	  OV8835MIPI_sensor.frame_height = OV8835MIPI_PV_PERIOD_LINE_NUMS + OV8835MIPI_sensor.dummy_lines + extra_shutter;
	...
	else
	{
	  ...
	  spin_lock_irqsave(&ov8835mipi_drv_lock,flags);
	  OV8835MIPI_sensor.frame_height = OV8835MIPI_FULL_PERIOD_LINE_NUMS + OV8835MIPI_sensor.dummy_lines + extra_shutter;
	  spin_unlock_irqrestore(&ov8835mipi_drv_lock,flags);
	}
	SENSORDB("OV8835MIPI_sensor.frame_height:%x \n",OV8835MIPI_sensor.frame_height);
	...
	if(OV8835MIPI_sensor.is_autofliker == KAL_TRUE)
		{
				...
				realtime_fp = pclk *10 / (OV8835MIPI_sensor.line_length * OV8835MIPI_sensor.frame_height);
			    SENSORDB("[OV8835MIPI_Write_Shutter]pv_clk:%d\n",pclk);
				SENSORDB("[OV8835MIPI_Write_Shutter]line_length:%d\n",OV8835MIPI_sensor.line_length);
				SENSORDB("[OV8835MIPI_Write_Shutter]frame_height:%d\n",OV8835MIPI_sensor.frame_height);
				...
				if((realtime_fp >= 297)&&(realtime_fp <= 303))
					{
						realtime_fp = 296;
						spin_lock_irqsave(&ov8835mipi_drv_lock,flags);
						OV8835MIPI_sensor.frame_height = pclk *10 / (OV8835MIPI_sensor.line_length * realtime_fp);
						spin_unlock_irqrestore(&ov8835mipi_drv_lock,flags);

						SENSORDB("[autofliker realtime_fp=30,extern heights slowdown to 29.6fps][height:%d]",OV8835MIPI_sensor.frame_height);
					}
				else if((realtime_fp >= 147)&&(realtime_fp <= 153))
					{
						...
						SENSORDB("[autofliker realtime_fp=15,extern heights slowdown to 14.6fps][height:%d]",OV8835MIPI_sensor.frame_height);
					}
				//OV8835MIPI_sensor.frame_height = OV8835MIPI_sensor.frame_height +(OV8835MIPI_sensor.frame_height>>7);
    ...
    OV8835MIPI_write_cmos_sensor(0x380e, (OV8835MIPI_sensor.frame_height>>8)&0xFF);
	OV8835MIPI_write_cmos_sensor(0x380f, (OV8835MIPI_sensor.frame_height)&0xFF);
...
//return;
	...
	{
	  line_length = OV8835MIPI_PV_PERIOD_PIXEL_NUMS + iPixels;
	  frame_height = OV8835MIPI_PV_PERIOD_LINE_NUMS + iLines;
	}
	else
	{
	  line_length = OV8835MIPI_FULL_PERIOD_PIXEL_NUMS + iPixels;
	  frame_height = OV8835MIPI_FULL_PERIOD_LINE_NUMS + iLines;
	}

	if(frame_height < (OV8835MIPI_sensor.shutter+14))
	{
		frame_height = OV8835MIPI_sensor.shutter +14;
	}
...
//	if((line_length == OV8835MIPI_sensor.line_length)&&(frame_height == OV8835MIPI_sensor.frame_height))
//		return ;
	spin_lock(&ov8835mipi_drv_lock);
	OV8835MIPI_sensor.line_length = line_length;
	OV8835MIPI_sensor.frame_height = frame_height;
    ...
    OV8835MIPI_write_cmos_sensor(0x380e, frame_height >> 8);
    OV8835MIPI_write_cmos_sensor(0x380f, frame_height & 0xFF);
...
{
	const kal_uint16 dummy_pixel = OV8835MIPI_sensor.line_length - OV8835MIPI_PV_PERIOD_PIXEL_NUMS;
	const kal_uint16 pv_min_fps =  enable ? OV8835MIPI_sensor.night_fps : OV8835MIPI_sensor.normal_fps;
	kal_uint16 dummy_line = OV8835MIPI_sensor.frame_height - OV8835MIPI_PV_PERIOD_LINE_NUMS;
	kal_uint16 max_exposure_lines;
	
#ifdef OV8835MIPI_DRIVER_TRACE
   SENSORDB("OV8835MIPI_night_mode:enable:%x;\n",enable);
#endif

	if (!OV8835MIPI_sensor.video_mode) return;
	max_exposure_lines = OV8835MIPI_sensor.pv_pclk * OV8835MIPI_FPS(1) / (pv_min_fps * OV8835MIPI_sensor.line_length);
	if (max_exposure_lines > OV8835MIPI_sensor.frame_height) /* fix max frame rate, AE table will fix min frame rate */
	{
	  dummy_line = max_exposure_lines - OV8835MIPI_PV_PERIOD_LINE_NUMS;
#ifdef OV8835MIPI_DRIVER_TRACE
	 SENSORDB("dummy_line:%x;\n",dummy_line);
#endif
	  OV8835MIPI_Set_Dummy(dummy_pixel, dummy_line);
	}
...
//#ifdef OV8835MIPI_DEBUG_SETTING
	...
	OV8835MIPI_write_cmos_sensor(0x0100, 0x00); //; software standby                                                                                     
	OV8835MIPI_write_cmos_sensor(0x0103, 0x01); //; software reset                                                                                       
	//delay(5,m0xs));                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x0102, 0x01);                                                                                                          
	//;system, 0xco);ntrol                                                                                                     
	OV8835MIPI_write_cmos_sensor(0x3001, 0x2a); //;drive 2x, d1_frex_in_disable                                                                          
	OV8835MIPI_write_cmos_sensor(0x3002, 0x88); //; vsync, strobe output enable, href, pclk, frex, SIOD output disable                                   
	OV8835MIPI_write_cmos_sensor(0x3005, 0x00);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x301b, 0xb4); //; sclk_bist20, sclk_snr_sync on                                                                        
	OV8835MIPI_write_cmos_sensor(0x301d, 0x02); //; frex_mask_arb on                                                                                     
	//OV8835MIPI_write_cmos_sensor(0x3021, 0x20); //; internal regulator on                                                                                
	OV8835MIPI_write_cmos_sensor(0x3022, 0x00); //; pad_pixelvdd_sel = 0                                                                                 
	 //  0x    , 0x  );                                                                                                          
	//;0xxpos,u0xre);/Gain                                                                                                     
	OV8835MIPI_write_cmos_sensor(0x3503, 0x07); //; AGC manual, AEC manual                                                                               
	OV8835MIPI_write_cmos_sensor(0x3504, 0x00);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3505, 0x30);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3506, 0x00); //; ShortExposure[10:16], for inter line HDR mode                                                        
	OV8835MIPI_write_cmos_sensor(0x3508, 0x80); //; ShortExposure[15:8]                                                                                  
	OV8835MIPI_write_cmos_sensor(0x3509, 0x10); //; ShortExposure[7:0]                                                                                   
	OV8835MIPI_write_cmos_sensor(0x350a, 0x00);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x350b, 0x38);                                                                                                          
	//;0xnalo,g0x  );                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3600, 0x78);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3601, 0x0a); //0x02-> 0x0a from tony                                                                                                         
	OV8835MIPI_write_cmos_sensor(0x3602, 0x9c); //0x1c->0x9c from tony                                                                                                         
	OV8835MIPI_write_cmos_sensor(0x3604, 0x38);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3620, 0x64);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3621, 0xb5);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3622, 0x03);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3625, 0x64);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3630, 0x55);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3631, 0xd2);                                                                                                          
	...
	OV8835MIPI_write_cmos_sensor(0x3680, 0xe0);                                                                                                          
	...
	//;0x OTP, 0x  );                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3d00, 0x00); //; OTP buffer                                                                                           
	...
	OV8835MIPI_write_cmos_sensor(0x3d0f, 0x00); //; OTP buffer                                                                                           
	OV8835MIPI_write_cmos_sensor(0x3d80, 0x00); //; OTP program                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3d81, 0x00); //; OTP load                                                                                             
	OV8835MIPI_write_cmos_sensor(0x3d84, 0x00); //; OTP program enable, manual memory bank disable, bank sel 0                                           
	//;0x BLC, 0x  );                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x4000, 0x18);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x4001, 0x04); //; BLC start line 4                                                                                     
	OV8835MIPI_write_cmos_sensor(0x4002, 0x45); //; BLC auto, reset frame number = 5                                                                     
	OV8835MIPI_write_cmos_sensor(0x4004, 0x02); //; 2 lines for BLC                                                                                      
	OV8835MIPI_write_cmos_sensor(0x4006, 0x16); //; DC BLC coefficient                                                                                   
	OV8835MIPI_write_cmos_sensor(0x4008, 0x20); //; First part BLC calculation start line address = 2, DC BLC on                                         
	OV8835MIPI_write_cmos_sensor(0x4009, 0x10); //; BLC target                                                                                           
	OV8835MIPI_write_cmos_sensor(0x4101, 0x12);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x4104, 0x5b);                                                                                                          
	//;0xform,a0xt );control                                                                                                   
	OV8835MIPI_write_cmos_sensor(0x4307, 0x30); //; embed_line_st = 3                                                                                    
	OV8835MIPI_write_cmos_sensor(0x4315, 0x00); //; Vsync trigger signal to Vsync output delay[15:8]                                                     
	OV8835MIPI_write_cmos_sensor(0x4511, 0x05);                                                                                                          
	//;0xMIPI, 0xco);ntrol                                                                                                     
	OV8835MIPI_write_cmos_sensor(0x4805, 0x01);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x4806, 0x00);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x481f, 0x36);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x4831, 0x6c);                                                                                                          
	//;0x LVD,S0x  );                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x4a00, 0xaa); //; SYNC code enable when only 1 lane, Bit swap, SAV first enable                                        
	OV8835MIPI_write_cmos_sensor(0x4a03, 0x01); //; Dummy data0[7:0]                                                                                     
	OV8835MIPI_write_cmos_sensor(0x4a05, 0x08); //; Dummy data1[7:0]                                                                                     
	OV8835MIPI_write_cmos_sensor(0x4a0a, 0x88);                                                                                                          
	//;0x ISP, 0x  );    
#ifdef OV8835MIPI_USE_OTP
	OV8835MIPI_write_cmos_sensor(0x5000, 0x86); //; LENC off, BPC on, WPC on   0x06->0x86 turn on lenc  
#else 
	OV8835MIPI_write_cmos_sensor(0x5000, 0x06); //; LENC off, BPC on, WPC on     
#endif
	OV8835MIPI_write_cmos_sensor(0x5001, 0x01); //; MWB on                                                                                               
	OV8835MIPI_write_cmos_sensor(0x5002, 0x80); //; scale on                                                                                             
	OV8835MIPI_write_cmos_sensor(0x5003, 0x20);                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x5013, 0x00); //; LSB disable                                                                                          
	OV8835MIPI_write_cmos_sensor(0x5046, 0x4a); //; ISP SOF sel - VSYNC                                                                                  
	//;0x DPC, 0x  );                                                                                                          
	...
	OV8835MIPI_write_cmos_sensor(0x5a08, 0x02); //; window control                                                                                       
	OV8835MIPI_write_cmos_sensor(0x5e00, 0x00); //; color bar off                                                                                        
	...
	//;0x MWB, 0x  );                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x5001, 0x01); //; MWB on                                                                                               
	OV8835MIPI_write_cmos_sensor(0x3400, 0x04); //; red gain h                                                                                           
	OV8835MIPI_write_cmos_sensor(0x3401, 0x00); //; red gain l                                                                                           
	OV8835MIPI_write_cmos_sensor(0x3402, 0x04); //; green gain h                                                                                         
	OV8835MIPI_write_cmos_sensor(0x3403, 0x00); //; green gain l                                                                                         
	OV8835MIPI_write_cmos_sensor(0x3404, 0x04); //; blue gain h                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3405, 0x00); //; blue gain l                                                                                          
	OV8835MIPI_write_cmos_sensor(0x3406, 0x01); //; MWB on                                                                                               
	//;0x BLC, 0x  );                                                                                                          
	OV8835MIPI_write_cmos_sensor(0x4000, 0x18); //; BLC on, BLC window as BLC, BLC ratio from register                                                   
	OV8835MIPI_write_cmos_sensor(0x4002, 0x45); //; Format change trigger off, auto on, reset frame number = 5                                           
	OV8835MIPI_write_cmos_sensor(0x4005, 0x18); //; no black line output, blc_man_1_en, BLC after reset, then stop, BLC triggered by gain change         
	//Daniel
	OV8835MIPI_write_cmos_sensor(0x404f, 0xaf);	//																	   
	//Daniel//
	OV8835MIPI_write_cmos_sensor(0x4009, 0x10); //; BLC target                                                                                           
	OV8835MIPI_write_cmos_sensor(0x3503, 0x07); //; AEC manual, AGC manual                                                                               
	OV8835MIPI_write_cmos_sensor(0x3500, 0x00); //; Exposure[19:16]                                                                                      
	OV8835MIPI_write_cmos_sensor(0x3501, 0x29); //; Exposure[15:8]                                                                                       
	OV8835MIPI_write_cmos_sensor(0x3502, 0x00); //; Exposure[7:0]                                                                                        
	OV8835MIPI_write_cmos_sensor(0x350b, 0x78); //; Gain[7:0]                                                                                            
	//;0x MIP,I0x d);ata rate = 640Mbps                                                                                        
	OV8835MIPI_write_cmos_sensor(0x30b3, 0x50); //; pll1_multiplier                                                                                      
	OV8835MIPI_write_cmos_sensor(0x30b4, 0x03); //; pll1_prediv                                                                                          
	OV8835MIPI_write_cmos_sensor(0x30b5, 0x04); //; pll1_op_pix_div                                                                                      
	OV8835MIPI_write_cmos_sensor(0x30b6, 0x01); //; pll1_op_sys_div                                                                                      
	OV8835MIPI_write_cmos_sensor(0x4837, 0x0c); //;0d ; MIPI global timing; //tony      
	...
	OV8835MIPI_write_cmos_sensor(0x301b, 0xb4);  //; sclk_bist20, sclk_snr_sync on		
	OV8835MIPI_write_cmos_sensor(0x301d, 0x02);  //; frex_mask_arb on	
	OV8835MIPI_write_cmos_sensor(0x3021, 0x00);  //; internal regulator on
	OV8835MIPI_write_cmos_sensor(0x3022, 0x02);  //; pad_pixelvdd_sel = 0		
	...
	OV8835MIPI_write_cmos_sensor(0x3098, 0x04);
	OV8835MIPI_write_cmos_sensor(0x3099, 0x10);
	...
	OV8835MIPI_write_cmos_sensor(0x350a, 0x00);
	//OV8835MIPI_write_cmos_sensor(0x350b, 0x38);
	OV8835MIPI_write_cmos_sensor(0x350b, 0x78);//for first enter sensor
	...
	OV8835MIPI_write_cmos_sensor(0x4001, 0x04);  //; BLC start line 4		
	OV8835MIPI_write_cmos_sensor(0x4002, 0x45);  //; BLC auto, reset frame number = 5  
//	OV8835MIPI_write_cmos_sensor(0x4005, 0x18);////for pre and cap diff
	OV8835MIPI_write_cmos_sensor(0x4005, 0x18);////for pre and cap diff
	//OV8835MIPI_write_cmos_sensor(0x4006, 0x20);  //; DC BLC coefficient  
	OV8835MIPI_write_cmos_sensor(0x4006, 0x1f);  //; DC BLC coefficient   //for pre and cap diff
//	OV8835MIPI_write_cmos_sensor(0x4008, 0x24);  //; First part BLC calculation start line address = 2, DC BLC on	
	OV8835MIPI_write_cmos_sensor(0x4008, 0x20);  //; First part BLC calculation start line address = 2, DC BLC on	//for pre and cap diff
	OV8835MIPI_write_cmos_sensor(0x4009, 0x10);  //; BLC target
...
OV8835MIPI_write_cmos_sensor(0x4009, 0x10);                             
...
OV8835MIPI_write_cmos_sensor(0x4006, 0x1f);
//
...
OV8835MIPI_1632_1224_2Lane_30fps_Mclk26M_setting(void)
{
	...
	//#ifdef OV8835MIPI_DEBUG_SETTING		
	#if 0
//	;//OV8835MIPI_1600*1200_setting_2lanes_640Mbps/lane_30fps			  
//	;//Base_on_OV8835MIPI_APP_R1.11 									  
//	;//2012_2_1 													  
//	;//Tony Li														  
//	;;;;;;;;;;;;;Any modify please inform to OV FAE;;;;;;;;;;;;;;;	  
	...
	OV8835MIPI_write_cmos_sensor(0x0100, 0x00);  // ; software standby						  
	OV8835MIPI_write_cmos_sensor(0x3708, 0xe2);  // ; sensor control							  
	OV8835MIPI_write_cmos_sensor(0x3709, 0x03);  // ; sensor control							  
	OV8835MIPI_write_cmos_sensor(0x3800, 0x00);  // ; HS = 8									  
	OV8835MIPI_write_cmos_sensor(0x3801, 0x08);  // ; HS										  
	OV8835MIPI_write_cmos_sensor(0x3802, 0x00);  // ; VS = 8									  
	OV8835MIPI_write_cmos_sensor(0x3803, 0x08);  // ; VS										  
	OV8835MIPI_write_cmos_sensor(0x3804, 0x0c);  // ; HE = 3287								  
	OV8835MIPI_write_cmos_sensor(0x3805, 0xd7);  // ; HE										  
	OV8835MIPI_write_cmos_sensor(0x3806, 0x09);  // ; VE = 2471								  
	OV8835MIPI_write_cmos_sensor(0x3807, 0xa7);  // ; VE										  
	OV8835MIPI_write_cmos_sensor(0x3808, 0x06);  // ; HO = 1632								  
	OV8835MIPI_write_cmos_sensor(0x3809, 0x60);  // ; HO										  
	OV8835MIPI_write_cmos_sensor(0x380a, 0x04);  // ; VO = 1224								  
	OV8835MIPI_write_cmos_sensor(0x380b, 0xc8);  // ; VO										  
	OV8835MIPI_write_cmos_sensor(0x380c, 0x0e);  // ; HTS = 3608								  
	OV8835MIPI_write_cmos_sensor(0x380d, 0x18);  // ; HTS 									  
	OV8835MIPI_write_cmos_sensor(0x380e, 0x04);  // ; VTS = 1260								  
	OV8835MIPI_write_cmos_sensor(0x380f, 0xec);  // ; VTS 									  
	OV8835MIPI_write_cmos_sensor(0x3810, 0x00);  // ; H OFFSET = 4							  
	OV8835MIPI_write_cmos_sensor(0x3811, 0x04);  // ; H OFFSET								  
	OV8835MIPI_write_cmos_sensor(0x3812, 0x00);  // ; V OFFSET = 4							  
	OV8835MIPI_write_cmos_sensor(0x3813, 0x04);  // ; V OFFSET								  
	OV8835MIPI_write_cmos_sensor(0x3814, 0x31);  // ; X INC									  
	OV8835MIPI_write_cmos_sensor(0x3815, 0x31);  // ; Y INC									  
	...
	//Daniel
	OV8835MIPI_write_cmos_sensor(0x4005, 0x18);  // Daniel: For solving color flushing issue											  
	OV8835MIPI_write_cmos_sensor(0x404f, 0xaf);	//																	   
	//Daniel//
//	OV8835MIPI_write_cmos_sensor(0x4005, 0x1a);  //											  
	OV8835MIPI_write_cmos_sensor(0x4512, 0x01);  // ; vertical average	  0x00 for sum					  
...
//	;PL0xL	 , 0x  );  //											  
	OV8835MIPI_write_cmos_sensor(0x3081, 0x02);  //											  
	OV8835MIPI_write_cmos_sensor(0x3083, 0x01);  //											  
	OV8835MIPI_write_cmos_sensor(0x3093, 0x00);  // ; pll2_sel5								  
	OV8835MIPI_write_cmos_sensor(0x3098, 0x03);  // ; pll3_prediv 							  
	OV8835MIPI_write_cmos_sensor(0x3099, 0x1d);  // ;1e; pll3_multiplier//tony				  
	OV8835MIPI_write_cmos_sensor(0x309a, 0x00);  // ; pll3_divs								  
	OV8835MIPI_write_cmos_sensor(0x309b, 0x01);  // ;00; pll3_div//tony						  
	OV8835MIPI_write_cmos_sensor(0x30a2, 0x01);  //											  
	OV8835MIPI_write_cmos_sensor(0x30b0, 0x05);  //											  
	OV8835MIPI_write_cmos_sensor(0x30b2, 0x00);  //											  
	OV8835MIPI_write_cmos_sensor(0x30b5, 0x04);  // ; pll1_op_pix_div 						  
	OV8835MIPI_write_cmos_sensor(0x30b6, 0x02);  // ; pll1_op_sys_div 						  
	OV8835MIPI_write_cmos_sensor(0x3104, 0xa1);  // ; sclk_sw2pll 							  
	OV8835MIPI_write_cmos_sensor(0x3106, 0x01);  //											  
//	; S0xCLK ,=0x 1);  //38.66Mhz									  
	OV8835MIPI_write_cmos_sensor(0x3090, 0x03);  // ; pll2_prediv 							  
	OV8835MIPI_write_cmos_sensor(0x3091, 0x20);  // ;23; pll2_multiplier//tony				  
	OV8835MIPI_write_cmos_sensor(0x3092, 0x01);  // ; pll2_divs								  
	OV8835MIPI_write_cmos_sensor(0x3093, 0x02);  // ;00; pll2_seld5//tony 					  
//	   0x	 , 0x  );  //											  

    //2012-02-27
    OV8835MIPI_write_cmos_sensor(0x30b6, 0x01);
	OV8835MIPI_write_cmos_sensor(0x3093, 0x01);
	OV8835MIPI_write_cmos_sensor(0x30b3, 0x3c);
	OV8835MIPI_write_cmos_sensor(0x4837, 0x0f);
    //2012-02-27
	OV8835MIPI_write_cmos_sensor(0x0100, 0x01);  // ; wake up
...
#if 0//ifdef OV8835MIPI_DEBUG_SETTING//1//OV8835MIPI_DEBUG_SETTING
	SENSORDB("OV8830MIPI_3264_2448_2Lane_15fps_Mclk26M_setting start \n");
																							   
	OV8835MIPI_write_cmos_sensor(0x0100, 0x00);	// ; software standby												   
	OV8835MIPI_write_cmos_sensor(0x3708, 0xe2);	// ; sensor control 												   
	OV8835MIPI_write_cmos_sensor(0x3709, 0x03);	// ; sensor control 												   
	OV8835MIPI_write_cmos_sensor(0x3800, 0x00);	// ; HS = 12														   
	OV8835MIPI_write_cmos_sensor(0x3801, 0x0c);	// ; HS 															   
	OV8835MIPI_write_cmos_sensor(0x3802, 0x00);	// ; VS = 12														   
	OV8835MIPI_write_cmos_sensor(0x3803, 0x0c);	// ; VS 															   
	OV8835MIPI_write_cmos_sensor(0x3804, 0x0c);	// ; HE = 3283														   
	OV8835MIPI_write_cmos_sensor(0x3805, 0xd3);	// ; HE 															   
	OV8835MIPI_write_cmos_sensor(0x3806, 0x09);	// ; VE = 2467														   
	OV8835MIPI_write_cmos_sensor(0x3807, 0xa3);	// ; VE 															   
	OV8835MIPI_write_cmos_sensor(0x3808, 0x0c);	// ; HO = 3264														   
	OV8835MIPI_write_cmos_sensor(0x3809, 0xc0);	// ; HO 															   
	OV8835MIPI_write_cmos_sensor(0x380a, 0x09);	// ; VO = 2448														   
	OV8835MIPI_write_cmos_sensor(0x380b, 0x90);	// ; VO 															   
	OV8835MIPI_write_cmos_sensor(0x380c, 0x0e);	// ; HTS = 3608 													   
	OV8835MIPI_write_cmos_sensor(0x380d, 0x18);	// ; HTS															   
	OV8835MIPI_write_cmos_sensor(0x380e, 0x09);	// ; VTS = 2484 													   
	OV8835MIPI_write_cmos_sensor(0x380f, 0xb4);	// ; VTS															   
	OV8835MIPI_write_cmos_sensor(0x3810, 0x00);	// ; H OFFSET = 4													   
	OV8835MIPI_write_cmos_sensor(0x3811, 0x04);	// ; H OFFSET														   
	OV8835MIPI_write_cmos_sensor(0x3812, 0x00);	// ; V OFFSET = 4													   
	OV8835MIPI_write_cmos_sensor(0x3813, 0x04);	// ; V OFFSET														   
	OV8835MIPI_write_cmos_sensor(0x3814, 0x11);	// ; X INC															   
	OV8835MIPI_write_cmos_sensor(0x3815, 0x11);	// ; Y INC															   
...
//	OV8835MIPI_write_cmos_sensor(0x4005, 0x1a);	//																	   
	...
	OV8835MIPI_write_cmos_sensor(0x404f, 0xaf);	//																	   
	//Daniel//
	OV8835MIPI_write_cmos_sensor(0x4512, 0x01);	// ; vertical average												   
	OV8835MIPI_write_cmos_sensor(0x3011, 0x21);	// ; MIPI 2 lane, MIPI enable										   
	OV8835MIPI_write_cmos_sensor(0x3015, 0xc8);	// ; MIPI 2 lane on, select MIPI									   
//	;PL0xL	 , 0x  );	//																	   
	OV8835MIPI_write_cmos_sensor(0x3081, 0x02);	//																	   
	...
	OV8835MIPI_write_cmos_sensor(0x3093, 0x00);	// ; pll2_sel5														   
	OV8835MIPI_write_cmos_sensor(0x3098, 0x03);	// ; pll3_prediv													   
	OV8835MIPI_write_cmos_sensor(0x3099, 0x1c);	// ;1e ; pll3_multiplier//Tony										   
	OV8835MIPI_write_cmos_sensor(0x309a, 0x00);	// ; pll3_divs														   
	OV8835MIPI_write_cmos_sensor(0x309b, 0x01);	// ;00 ; pll3_div//Tony 											   
	OV8835MIPI_write_cmos_sensor(0x30a2, 0x01);	//																	   
	OV8835MIPI_write_cmos_sensor(0x30b0, 0x05);	//																	   
	OV8835MIPI_write_cmos_sensor(0x30b2, 0x00);	//																	   
	OV8835MIPI_write_cmos_sensor(0x30b3, 0x48);	//Tony_for 13fps
	OV8835MIPI_write_cmos_sensor(0x30b5, 0x04);	// ; pll1_op_pix_div												   
	OV8835MIPI_write_cmos_sensor(0x30b6, 0x01);	// ; pll1_op_sys_div												   
	OV8835MIPI_write_cmos_sensor(0x3104, 0xa1);	// ; sclk_sw2pll													   
	OV8835MIPI_write_cmos_sensor(0x3106, 0x01);	//																	   
//	; S0xCLK ,=0x 1);	//36Mhz 															   
	OV8835MIPI_write_cmos_sensor(0x3090, 0x03);	// ; pll2_prediv													   
	OV8835MIPI_write_cmos_sensor(0x3091, 0x1f);	// ;22 ; pll2_multiplier//Tony										   
	OV8835MIPI_write_cmos_sensor(0x3092, 0x01);	// ; pll2_divs														   
	OV8835MIPI_write_cmos_sensor(0x3093, 0x01);	// ;00 ; pll2_seld5//Tony	
    OV8835MIPI_write_cmos_sensor(0x4837, 0x0d);	// tony_for 13fps	
...
void OV8835MIPI_3264_2448_2Lane_13fps_Mclk26M_setting(void)
{
...
//	;;;;;;;;;;;;;Any modify please inform to OV FAE;;;;;;;;;;;;;;;	
	...
	SENSORDB("OV8835MIPI_3264_2448_2Lane_13fps_Mclk26M_setting start \n");
...
//	;PL0xL	 , 0x  );	//																	   
	...
	OV8835MIPI_write_cmos_sensor(0x30b0, 0x05);	//																	   
	OV8835MIPI_write_cmos_sensor(0x30b2, 0x00);	//																	   
	OV8835MIPI_write_cmos_sensor(0x30b3, 0x40);	//Tony_for 13fps
	OV8835MIPI_write_cmos_sensor(0x30b5, 0x04);	// ; pll1_op_pix_div												   
	OV8835MIPI_write_cmos_sensor(0x30b6, 0x01);	// ; pll1_op_sys_div												   
	OV8835MIPI_write_cmos_sensor(0x3104, 0xa1);	// ; sclk_sw2pll													   
	OV8835MIPI_write_cmos_sensor(0x3106, 0x01);	//																	   
//	; S0xCLK ,=0x 1);	//36Mhz 															   
	OV8835MIPI_write_cmos_sensor(0x3090, 0x03);	// ; pll2_prediv													   
	OV8835MIPI_write_cmos_sensor(0x3091, 0x1b);	// ;22 ; pll2_multiplier//Tony										   
	OV8835MIPI_write_cmos_sensor(0x3092, 0x01);	// ; pll2_divs														   
	OV8835MIPI_write_cmos_sensor(0x3093, 0x01);	// ;00 ; pll2_seld5//Tony	
    OV8835MIPI_write_cmos_sensor(0x4837, 0x0e);	// tony_for 13fps	
...
#endif
#endif
	SENSORDB("OV8835MIPI_3264_2448_2Lane_13fps_Mclk26M_setting end \n");
...
UINT32 OV8835MIPIOpen(void)
{
  ...
  for(i = 0; i <(sizeof(sccb_writeid)/sizeof(sccb_writeid[0])); i++)
   ...
   printk("OV8835MIPI_update_wb_register_from_otp();\n");
    ...
    if(1 == ret)
    {
        printk("OV8835MIPI_update_wb_register_from_otp invalid\n");
    }
    else if(0 == ret)
    {
        printk("OV8835MIPI_update_wb_register_from_otp success\n");
    }
    */
    //printk("OV8835MIPI_update_lenc_register_from_otp();\n");
    ret = ov8835mipi_update_lenc_register_from_otp();
    if(1 == ret)
    {
        printk("OV8835MIPI_update_lenc_register_from_otp invalid\n");
    }
    else if(0 == ret)
    {
        printk("OV8835MIPI_update_lenc_register_from_otp success\n");
    }
    ...
    {
        printk("OV8830MIPI_update_wb_register_from_otp success\n");
    }
    ov8835mipi_update_blc_register_from_otp();
    ov8835mipi_update_af_from_otp();
#endif
   return ERROR_NONE;
}   /* OV8835MIPIOpen  */
	...
	if(bEnable)
		{
			...
			//	{
			//		pv_max_frame_rate_lines = OV8835MIPI_sensor.frame_height + (OV8835MIPI_sensor.frame_height>>7);
	...
	else
		{
			...
			//if(OV8835MIPI_sensor.video_mode = KAL_TRUE)
			//	{
			//		pv_max_frame_rate_lines = OV8835MIPI_sensor.frame_height;
			//		OV8835MIPI_write_cmos_sensor(0x380e, (pv_max_frame_rate_lines>>8)&0xFF);
			//		OV8835MIPI_write_cmos_sensor(0x380f, (pv_max_frame_rate_lines)&0xFF);
			//	}	
		}
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_GET_PERIOD:	/* 3 */
			switch(OV8835MIPIMIPIRAWCurrentScenarioId)
				{
					case MSDK_SCENARIO_ID_CAMERA_ZSD:
						*pFeatureReturnPara16++= OV8835MIPI_FULL_PERIOD_PIXEL_NUMS + OV8835MIPI_sensor.dummy_pixels;
						*pFeatureReturnPara16= OV8835MIPI_FULL_PERIOD_LINE_NUMS + OV8835MIPI_sensor.dummy_lines;
						*pFeatureParaLen=4;
			            #ifdef OV8835MIPI_DRIVER_TRACE
						...
						break;
					default:
						*pFeatureReturnPara16++= OV8835MIPI_PV_PERIOD_PIXEL_NUMS + OV8835MIPI_sensor.dummy_pixels;
						*pFeatureReturnPara16= OV8835MIPI_PV_PERIOD_LINE_NUMS + OV8835MIPI_sensor.dummy_lines;
						*pFeatureParaLen=4;
			            #ifdef OV8835MIPI_DRIVER_TRACE
				          SENSORDB("SENSOR_FEATURE_GET_PERIOD£¬OV8835MIPI pv line length:%d\n",OV8835MIPI_PV_PERIOD_PIXEL_NUMS + OV8835MIPI_sensor.dummy_pixels); 
			            #endif	
						break;
				}		
		break;
