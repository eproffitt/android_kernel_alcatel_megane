...
//
#else
//pcyang added for test battery charge
#define YANG_DEBUG	0
//#define BAT_FULL_CURRENT
#if(1==YANG_DEBUG)
int g_battery_id=800;	//it is used to debug batt_id check when charge by software
int g_battery_temperature=10;	//it is used to debug batt_temp check charge by software
#endif

#ifndef TARGET_BUILD_MMITEST
#define BATT_ID_CHECK_SUPPORT 1 //pcyang added for check battery id
#endif

#ifdef TARGET_BUILD_MMITEST
#define NO_DETECT_BATTERY_ID_TEMP
#endif
extern kal_int32 gFG_booting_counter_I_FLAG;
int g_fg_ready_flag = 0;
int high_temp_stop_charge_flag=0;
///////////////////////////////////////////////////////////////////////////////////////////
...
#define TEMPERATURE_GT_ZERO_OFFSET 30		//convert temperature > 0
...
#define BATTERY_TEMP_AVERAGE_SIZE 	12
#define RECHARGING_HIGH_TEMP_VOLTAGE	4000
static int batteryTempIndex = 0;
static int batteryTempBuffer[BATTERY_AVERAGE_SIZE]; // only use BATTERY_TEMP_AVERAGE_SIZE byte. donot use sizeof.
...
int CHARGING_FULL_CURRENT=220;    // mA modified from 220 to 130 by pcyang
...
#define POST_CHARGING_HIGH_TEMP_TIME 1*60 //1mins
...
void check_battery_id(void){
  ...
  printk(KERN_INFO "%s: bat_id_vol:%d!\n", __func__, bat_id_vol);
  printk(KERN_INFO "bat_id_vol:%d!\n", bat_id_vol);
  #if(1==YANG_DEBUG)
  bat_id_vol=g_battery_id;
  printk(KERN_INFO "%s: g_battery_id:%d!\n", __func__,bat_id_vol);
  printk(KERN_INFO "g_battery_id:%d!\n",bat_id_vol);
  #endif
  if((bat_id_vol < CHG_MIN_VALID_BATT_ID) || (bat_id_vol > CHG_MAX_VALID_BATT_ID)) /*battery id error*/
       {
	printk(KERN_INFO "battery id error %d!\n", bat_id_vol);
...
/* ZhaiQingbo modify begin for factory software booting up normally without battery ID and temperature pin, 2013.8.1 */
#ifdef NO_DETECT_BATTERY_ID_TEMP
    bat_data->BAT_batt_temp= 200; //20degree*10
#else
            ...
            else 
            {
                ...
                else
                {
                    ...
                    {                        
                        ...
                        bat_data->BAT_CAPACITY = bat_volt_check_point;
                        if(high_temp_stop_charge_flag==1)
                        {
    ...
    if (gFG_booting_counter_I_FLAG == 2) {
        if (bat_volt_check_point == 1) {
            ...
            xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[Battery] mt6329_battery_update FG RTC:%d. \r\n", bat_volt_check_point);
...
#define BAT_NTC_TSM_1  1		// for scud battery. for aws
...
BATT_TEMPERATURE Batt_Temperature_Table[] = {
    ...
    else if(Res <= Batt_Temperature_Table[80].TemperatureR)
    {
        TBatt_Value = 60;
    }
    else
    {
        ...
        for (i = 0; i <= 80; i++)
        {
            if(Res >= Batt_Temperature_Table[i].TemperatureR)
            {
				...
				#if 1		// 44.5-->45, 44.4-->44
				if (Res > (Batt_Temperature_Table[i].TemperatureR + Batt_Temperature_Table[i-1].TemperatureR)/2){
					TBatt_Value = i-1;
				}else{
					TBatt_Value = i;
				}
...
void pchr_turn_off_charging (void)
{
    ...
    #if(1==YANG_DEBUG)
    xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[BATTERY] pchr_turn_off_charging is called!\r\n");
    #endif
...
{
    #if(1==YANG_DEBUG)
    printk("<0> pcyang:%s:BMT_status.bat_charging_state =%x\n",__func__,BMT_status.bat_charging_state );
    WARN_ON(1);
    #endif
    ...
    else
    {
	    ...
	    /*pcyang added for high temp charge ====2013.07.09*/
	     if ((BMT_status.temperature > 0)&&(BMT_status.temperature <= 38))
            {
                upmu_chr_vbat_cv_vth(0x0);  //VBAT_CV_VTH,default 4.2v
            }
            ...
            {
                upmu_chr_vbat_cv_vth(0x18); //0x18--4.1v
            }
            /*pcyang added end*/
#endif
...
int BAT_CheckBatteryStatus(void)
{
    ...
    else
    {
		...
		batteryTempSum = 0;
		temp_max = g_bat_temperature_pre;
		temp_min = g_bat_temperature_pre;
		for (i=0; i<BATTERY_TEMP_AVERAGE_SIZE; i++){
			if (batteryTempBuffer[i] > temp_max){
				temp_max = batteryTempBuffer[i];
			}
			if (batteryTempBuffer[i] < temp_min){
				temp_min = batteryTempBuffer[i];
			}
			batteryTempSum += batteryTempBuffer[i];
		}
		
		batteryTempSum += TEMPERATURE_GT_ZERO_OFFSET * BATTERY_TEMP_AVERAGE_SIZE;
		batteryTempSum -= TEMPERATURE_GT_ZERO_OFFSET + temp_max;
		batteryTempSum -= TEMPERATURE_GT_ZERO_OFFSET + temp_min;

		//0.5-->1, 0.4-->0
		batteryTempSum = (batteryTempSum * 10 + 5 * (BATTERY_TEMP_AVERAGE_SIZE -2))/(BATTERY_TEMP_AVERAGE_SIZE -2);
		BMT_status.temperature = batteryTempSum/10 - TEMPERATURE_GT_ZERO_OFFSET;
...
#endif
#if(1==YANG_DEBUG)
    BMT_status.temperature=g_battery_temperature;	// is used to test temprature charge by software.Don't open it if you have to
#endif

/*pcyang added for battery temprature check*/
...
#endif        
        {/*modified by pcyang for high temp charge ---2013-07-18*/
            if((BMT_status.temperature >= 45)&&(BMT_status.temperature <50))
            {
            ...
            /*pcyang modified end  --2013-07-18*/
        }
    }
...
void mt_battery_notify_check(void)
{
    ...
    if(g_BN_TestMode == 0x0000)
    {
        ...
        if((BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)||(high_temp_stop_charge_flag==1))
...
void BAT_thread(void)
{
    ...
    if( (g_battery_thermal_throttling_flag==1) || (g_battery_thermal_throttling_flag==3) )
    {
        ...
        if(BMT_status.temperature >= 60 || BMT_status.temperature <=-20)
        {
            ...
            */
/* ZhaiQingbo modify begin for factory software booting up normally without battery ID and temperature pin, 2013.8.1 */
#ifndef NO_DETECT_BATTERY_ID_TEMP
            mt_power_off();
            //#endif
#endif
        }
    ...
    if( upmu_is_chr_det() == KAL_TRUE )
    {
		...
		while( (detect_counter < 3) && (CHR_Type_num == NONSTANDARD_CHARGER) ){
			detect_counter++;
			CHR_Type_num = hw_charger_type_detection();     /*this function may consume 100ms*/
			BMT_status.charger_type = CHR_Type_num;	/*added by pcyang for detect ac charger exactly --2013-04-29*/
    ...
    //printk("<0> pcyang:BMT_status.charger_protect_status=%x",BMT_status.charger_protect_status);
    ...
    else
    {
        ...
        else
        {
            ...
            {
                /*pcyang add for high temp charge stop ---2013-07-18*/
                if((BMT_status.temperature >= 45)&&(BMT_status.temperature <50)&&(post_charging_time >= POST_CHARGING_HIGH_TEMP_TIME))
                {
                        BMT_status.bat_charging_state = CHR_BATFULL;
                        BAT_BatteryFullAction();
                        high_temp_stop_charge_flag=1;
                        post_charging_time=0;
                        return;
                }
                else
...
{
    kal_int32 battery_temp=g_battery_temperature;
     xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] battery temp : %d\n", battery_temp);
     return sprintf(buf, "%u\n", battery_temp);
}

static ssize_t store_battery_temperature(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    kal_int32 battery_temp=0;
    int rc=0;
    rc=kstrtoint(buf,10,&battery_temp);
    g_battery_temperature=battery_temp;
    return size;
}

static DEVICE_ATTR(BATT_ID, 0664, show_battery_id, store_battery_id);
static DEVICE_ATTR(BATT_TEMP, 0664, show_battery_temperature, store_battery_temperature);
#endif

#ifdef BAT_FULL_CURRENT
static ssize_t show_battery_full_current(struct device *dev,struct device_attribute *attr, char *buf)
{
	kal_int32 battery_full_current=CHARGING_FULL_CURRENT;
	
	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] battery full current[CURRENT] : %d\n", battery_full_current);

	return sprintf(buf, "%u\n", battery_full_current);
}

static ssize_t store_battery_full_current(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	kal_int32 battery_full_current=0;
	int rc=0;

	rc=kstrtouint(buf,10,&battery_full_current);

	CHARGING_FULL_CURRENT=battery_full_current;

	xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] battery full current[NEW] : %d\n", battery_full_current);

	return size;
}

static DEVICE_ATTR(BATT_FULL_CURRENT, 7777, show_battery_full_current, store_battery_full_current);
#endif
static ssize_t show_charge_type(struct device *dev,struct device_attribute *attr, char *buf)
{
    kal_int32 charge_type=BMT_status.charger_type;
     xlog_printk(ANDROID_LOG_DEBUG, "Power/Battery", "[EM] charge_type : %d\n", charge_type);
     return sprintf(buf, "%u\n", charge_type);
}
...
static int mt6329_battery_probe(struct platform_device *dev)    
{
    ...
    #if(1==YANG_DEBUG)
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_BATT_TEMP);
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_BATT_ID);
    #endif
    #ifdef BAT_FULL_CURRENT
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_BATT_FULL_CURRENT);
    #endif
